...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            + 'VERSION:3.0\n' 
            + 'FN:›㪘 𝐿𝛥͢𝛵𝑋 – 𝐹𝑅𝛩𝑆𝛵𝑌 デッド 𖧄‹\n' // full name
            + 'ORG:Dono Da Zero\n' // the organization of the contact
            + 'TEL;type=CELL;type=VOICE;waid=5519996503657:+55 (19) 99650-3657\n' // WhatsApp ID + phone number
            + 'END:VCARD'
prefix = botPrefix
blocked = []
	...
	nzwa.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: 'Espere..',
				success: 'Sucesso!',
                                levelon: '✔️ *Leveling ativo!*',
				leveloff: ' ✔️  *Leveling desativado!*',
				levelnoton: '❌­ *Leveling desativado!*',
				levelnol: '*KKKKKKKKKJ LEVEL 0 :/*',
				error: {
				...
				only: {
					group: '[❕] Apenas grupos!!',
					ownerG: '[❕] Apenas dono do grupo!!',
					ownerB: '[❕] Apenas dono do bot!',
					admin: '[❕] Apenas admins!!',
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '5519996503657@s.whatsapp.net'
                ...
                let found = false
                    for (let lmt of _limit) {
                        if (lmt.id === sender) {
                            ...
                            if (limitCounts <= 0) return nzwa.sendMessage(from,`Seu limite acabou\n\n_Nota : o limite é resetado as 21:00!_`, text,{ quoted: mek})
		...
		if (isGroupAdmins) return reply('Porque é um admin, não irei bani-lo..')
		nzwa.updatePresence(from, Presence.composing)
		if (messagesC.includes("#zero")) return reply("#zero on")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`Link de grupo detectado ${sender.split("@")[0]} Você será expulso do grupo por mais 5 segundos`)
		setTimeout( () => {
			nzwa.groupRemove(from, [kic]).catch((e)=>{reply(`*ERRO:* ${e}`)})
		}, 5000)
		setTimeout( () => {
			nzwa.updatePresence(from, Presence.composing)
			reply("1 segundo")
		...
		nzwa.sendMessage(from, bahasa(prefix, sender), text, {quoted: mek})
               ...
               break
				case 'demote':
					...
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Para isso marque um admin!')
					...
					if (mentioned.length > 1) {
						...
						mentions(`@${mentioned[0].split('@')[0]} virou membro comum!`)
						nzwa.groupDemoteAdmin(from, mentioned)
					}
					break
                                case 'randomhentai':
                  ...
                  case 'promote':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Marque alguem para isso!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						...
						mentions(`Pedido recebido, adicionando departamento como admin : @${mentioned[0].split('@')[0]}`, mentioned, true)
						nzwa.groupMakeAdmin(from, mentioned)
					}
					break
				  case 'wa.me':
  ...
  nzwa.updatePresence(from, Presence.composing) 
    ...
    nzwa.sendMessage(from, options, text, { quoted: mek } )
			...
			case 'quotes':
              ...
              case 'owner':
                case 'creator':
                  nzwa.sendMessage(from, {displayname: "›'㪘 𝐿𝛥͢𝛵𝑋 – 𝐹𝑅𝛩𝑆𝛵𝑌デッド 𖧄'‹", vcard: vcard}, MessageType.contact, { quoted: mek})
               nzwa.sendMessage(from, 'Numero do meu criador!',MessageType.text, { quoted: mek} )
                break
	case 'hidetag':
                nzwa.updatePresence(from, Presence.composing) 
                if (!isRegister) return reply(mess.only.daftarB)
              ...
              await nzwa.sendMessage(from, options, text)
               break
                                case 'tiktokstalk':
					try {
						if (args.length < 1) return nzwa.sendMessage(from, 'User? ', text, {quoted: mek})
                                                if (!isRegister) return reply(mess.only.daftarB)
                                                if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				case 'snowwrite':
					...
					var gbl7 = gh.split("|")[0];
					var gbl8 = gh.split("|")[1];
					if (args.length < 1) return reply(`Para isso use ${prefix}snowwrite texto1|texto2`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					reply(mess.wait)
		...
		case 'infonomor':
               nzwa.updatePresence(from, Presence.composing) 
                ...
                break
                   ...
                   nzwa.sendMessage(from, hasil, image, {quoted: mek, caption: `Mapa de *${body.slice(5)}*`})
                   await limitAdd(sender)
                   break
                   case 'covidcountry':
                   nzwa.updatePresence(from, Presence.composing) 
				...
				case 'ytmp3':
					...
					teks = `*Título* : ${anu.title}\n *Tamanho* : ${anu.filesize}\n\nSua midia será enviada em breve, aguarde...`
				...
				case 'ytmp4':
					...
					if (anu.error) return reply(anu.error)
					teks = `*Título* : ${anu.title} \n*Tamanho* : ${anu.filesize} \n*Desc* : ${anu.desc} \n\n*O vídeo será enviado em breve, aguarde...*`
					thumb = await getBuffer(anu.thumb)
					nzwa.sendMessage(from, thumb, image, {quoted: mek, caption: teks})
					buffer = await getBuffer(anu.result)
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				case 'tts':
				   ...
				   if (args.length < 1) return nzwa.sendMessage(from, 'E o código?', text, {quoted: mek})
                                   if (!isRegister) return reply(mess.only.daftarB)
                                   if (isLimit(sender)) return reply(ind.limitend(pusname))
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return nzwa.sendMessage(from, 'E o texto?', text, {quoted: mek})
					dtt = body.slice(8)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 600
				...
				case 'adminlist':
					nzwa.updatePresence(from, Presence.composing) 
                                        if (!isRegister) return reply(mess.only.daftarB)
					if (!isGroup) return reply(mess.only.group)
					teks = `Admins da *${groupMetadata.subject}*\nTotal : ${groupAdmins.length}\n\n`
					no = 0
				...
				case 'everyone':
					...
					mentions('Chamando todos os cornos para '+teks+'(?)', members_id, true)
					break
                case 'tagall2':
				nzwa.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
					...
					nzwa.sendMessage(from, 'Chamando todos os cornos para '+teks+'(?)', text, {quoted: mek})
					break
                case 'tagall3':
				nzwa.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
					...
					nzwa.sendMessage(from, 'Chamando todos os cornos para '+teks+'(?)', text, {detectLinks: false, quoted: mek})
					break
                        case 'tagall4':
				nzwa.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				nzwa.updatePresence(from, Presence.composing) 
					...
					reply('Chamando todos os cornos para '+teks+'(?)')
					break
				case 'send':
					var pc = body.slice(6)
				...
				case 'chord':
					...
					break
				case 'lirik':
					if (args.length < 1) return reply('Musica?')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					tels = body.slice(7)
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           break
				...
				case 'add':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('Quem vou adicionar?')
					try {
						num = `${args[0].replace(/ /g, '')}@s.whatsapp.net`
						nzwa.groupAdd(from, [num])
					} catch (e) {
						console.log('Erro :', e)
						reply('Possivelmente conta privada!')
					}
					break
				...
				case 'kick':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Para isso marque alguém!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						...
						nzwa.groupRemove(from, mentioned)
					nzwa.sendMessage(mentioned, 'Tchau tchau‚', text)
					}
					break
				case 'exe':
	              nzwa.updatePresence(from, Presence.composing) 
	    ...
	    case 'kapankah':
			...
			case 'closegc':
                ...
                case 'bukagc':
					...
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					open = {
					text: `Grupo aberto @${sender.split("@")[0]}`,
					contextInfo: { mentionedJid: [sender] }
					}
